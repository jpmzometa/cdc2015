#ifndef AIRCRAFTPCEMTXOPS_H
#define AIRCRAFTPCEMTXOPS_H
 ...
 * pmtx has size (rows x columns) */
extern void aircraftpce_mtx_scale(real_t pout[], const real_t pmtx[],
		const real_t factor, const uint32_t rows,
		const uint32_t cols);

/* matrix addition: pmtxc = pmta + pmtxb.
 * pmta, pmtb, pmtc have size (rows x columns) */
extern void aircraftpce_mtx_add(real_t pmtxc[], const real_t pmtxa[],
		const real_t pmtxb[], const uint32_t rows,
		const uint32_t cols);

/* matrix substraction: pmtxc = pmta - pmtxb.
 * pmta, pmtb, pmtc have size (rows x columns) */
extern void aircraftpce_mtx_substract(real_t pmtxc[], const real_t pmtxa[],
		const real_t pmtxb[], const uint32_t rows,
		const uint32_t cols);
 ...
 * pvecab = pmtxa * pvecb */
extern void aircraftpce_mtx_mul_add(real_t pvecc[], real_t pvecab[], const real_t pmtxa[],
		const real_t pvecb[], const uint32_t rows,
		const uint32_t cols);

/* elementwise 2-sided vector saturation. 
 * lower[i] < vec[i] < upper[i]. pvec has size (rows x 1) */
extern void aircraftpce_mtx_saturate_vec(real_t pvec[], const real_t plower[],
		const real_t pupper[],
		const uint32_t rows);

/* matrix transposition. mtxout[j,i] = mtxin[i,j].
 * mtxin has size (rows x columns) */
extern void aircraftpce_mtx_transpose(real_t * mtxout, const real_t * mtxin,
		const uint32_t rows, const uint32_t cols);

/* elementwise vector maximum against zero
 * pmax[i] = max(pa[i], 0). pmax has size (rows x 1) */
extern void aircraftpce_mtx_max_vec_zero(real_t pmax[],	const uint32_t rows);

/* elementwise vector minimum against zero
 *  pmin[i] = min(pa[i], 0). pmin has size (rows x 1) */
extern void aircraftpce_mtx_min_vec_zero(real_t pmin[], const uint32_t rows);
