#include "include/aircraftpcemtxops.h"
#ifdef AIRCRAFTPCEFIPOPS
 ...
 * pmtx has size (rows x columns) */
void aircraftpce_mtx_scale(real_t pout[], const real_t pmtx[],
...
void aircraftpce_mtx_add(real_t pmtxc[], const real_t pmtxa[],
 ...
 * pmta, pmtb, pmtc have size (rows x columns) */
void aircraftpce_mtx_substract(real_t pmtxc[], const real_t pmtxa[],
		const real_t pmtxb[], const uint32_t rows,
		const uint32_t cols)
{
	uint32_t k; /* matrix index (row * column) */
...
void aircraftpce_mtx_mul_add(real_t pvecc[], real_t pvecab[], const real_t pmtxa[],
		const real_t pvecb[], const uint32_t rows,
		const uint32_t cols)
{
	uint32_t k; /* vector index (rows) */
  aircraftpce_mtx_multiply_mtx_vec(pvecab, pmtxa, pvecb, rows, cols);
	for (k = 0; k < rows; k++) {
		pvecc[k] += pvecab[k];
	}
	return;
...
void aircraftpce_mtx_saturate_vec(real_t pvec[], const real_t plower[],
		const real_t pupper[],
		const uint32_t rows)
{
...
void aircraftpce_mtx_max_vec_zero(real_t pmax[], const uint32_t rows)
{
	uint32_t i; /* vector index (row number) */
	real_t zero = 0.;
	for (i = 0; i < rows; i++) {
...
void aircraftpce_mtx_min_vec_zero(real_t pmin[], const uint32_t rows)
{
	uint32_t i; /* vector index (row number) */
	real_t zero = 0.;
	for (i = 0; i < rows; i++) {
...
void aircraftpce_mtx_transpose(real_t * mtxout, const real_t * mtxin,
